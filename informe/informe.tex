...
\begin{itemize}
\item Para cerrar la simulación presione \emph{q}.
\item Para hacer zoom utilice la rueda del mouse.
\item Para mover la cámara mantenga presionado el click izquierdo y mueva el mouse.
\item Para aumentar/disminuir la velocidad presione \emph{+}/\emph{-} del teclado númerico o bien \emph{w}/\emph{s}.
\end{itemize}
...
data Wire s e m a b
\end{lstlisting}
Un Wire representa una `función' reactiva\footnote{Que varía con el tiempo.}. Más específicamente, es una función que toma de argumentos:
\begin{itemize}
...
\begin{itemize}
\item O bien devuelve un valor de tipo $a$ o se `inhibe' con un valor de tipo $e$ (\textbf{e}rror).
...
\end{lstlisting}
Notar como al devolver una nueva instancia de sí mismo, el Wire puede variar su comportamiento a medida que pasa el tiempo.
\newline
...
La simulación se hizo aprovechando los Wires. En particular los mismos permiten estructurar el sistema en ``cajas negras'' que se pueden combinar para crear cajas más complejas, donde no es necesario saber que pasa dentro de cada una\footnote{con la salvedad de que se sabe sí hay side-effects o no.}.
El siguiente esquema muestra de forma simplificada la estructura de la simulación. Cada caja representa un Wire.
...
\draw
	...
	\draw[->] (7,3.11) |- (f2);
	\draw[->] (7,3.11) |- (f1);
	\draw[->] (speedHandler) -- node[near end]{speed}(7, 2);
...
Como se puede apreciar en el diagrama, la velocidad afecta a la simulación tanto como al renderizado. Más específicamente cuando se aumenta la velocidad:
\begin{itemize}
